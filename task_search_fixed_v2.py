#!/usr/bin/env python3
"""
Plain-English task search with stricter primary-object matching.

Use this test version when searches like "remove basin" must not pick unrelated
items such as "remove radiator" just because both contain the word remove.
"""
...
PRIMARY_OBJECTS = {
    "basin", "radiator", "toilet", "tap", "shower", "bath", "valve", "cistern", "waste", "trap",
...
def tokens(text):
    return [OBJECT_SYNONYMS.get(t, t) for t in norm(text).split() if t and t not in STOPWORDS]
...
def query_parts(query):
    ...
    original_tokens = set(tokens(query))
    actions = q_tokens & ACTION_WORDS
    primary_objects = (original_tokens & PRIMARY_OBJECTS) or (q_tokens & PRIMARY_OBJECTS)
    generic_objects = q_tokens & GENERIC_OBJECTS
    return q_tokens, actions, primary_objects, generic_objects
...
def score_task(query, task):
    ...
    q_tokens, q_actions, q_primary_objects, q_generic_objects = query_parts(query)
    ...
    score += len(q_tokens & h_tokens) * 15

    # Primary object must dominate. Generic pipework/cap-off words are not enough.
    if q_primary_objects:
        primary_overlap = q_primary_objects & h_tokens
        if primary_overlap:
            score += 140 + (len(primary_overlap) * 35)
        else:
            score -= 260
    elif q_generic_objects:
        generic_overlap = q_generic_objects & h_tokens
        if generic_overlap:
            score += 35
    ...
    if q_actions:
        ...
        if action_overlap:
            score += 65 + (len(action_overlap) * 20)
        else:
            score -= 50
    ...
    score += int(SequenceMatcher(None, q_expanded, name_norm).ratio() * 10)
    score += int(SequenceMatcher(None, q_expanded, code_norm).ratio() * 4)
    return score


def best_matches(query, tasks, limit=8):
    ranked = [(score_task(query, task), task) for task in tasks]
    ranked = [(score, task) for score, task in ranked if score > 0]
